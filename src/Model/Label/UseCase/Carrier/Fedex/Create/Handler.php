...
class Handler
{
    ...
    {
        $existsCustom = $this->carrierRepository->existsCustom($command->modifiedCompany, TypeEnum::FEDEX);
        ...
        $carrierEasyPost = $this->easyPostClient->createCarrier($carrierFedexDto);

        $carrier = new Carrier(
            Id::next(),
            new Name($carrierFedexDto->getName()),
            Type::fedex(),
            new Description($carrierFedexDto->getDescription()),
            new CarrierAccount($carrierEasyPost->id),
            new Credentials($carrierFedexDto->getCredentials()),
            Custom::yes(),
            Editable::yes(),
            Status::active(),
